...
# 1000 lượt quay - Quay thử
class YYThousandDraw(RequestHandlerTask):
	url = r'/game/yy/thousand/draw'
	...
	def run(self):
		...
		if yyID is None or idx is None:
			raise ClientError('param miss')

		role = self.game.role
		record = role.yyhuodongs.setdefault(yyID, {})
		info = record.setdefault('info', {})
		info.setdefault('round', 1)
		choose = record.setdefault('choose', {})

		import random
		pool = []
		for card_id, card_cfg in csv.cards.iteritems():
			try:
				unit_id = card_cfg.unitID
				unit_cfg = csv.unit[unit_id]
				if unit_cfg.rarity >= 3 and card_cfg.cardType == 1:
					pool.append(card_id)
			except Exception:
				pass

		if not pool:
			pool = [1001, 1002, 1003, 1004, 1005, 1006, 1007, 1008, 1009, 1010]

		cids = random.sample(pool, min(10, len(pool)))
		choose[str(idx)] = cids
		role.yyhuodongs[yyID] = record

		self.write({'view': {}})


# 1000 lượt quay - Nhận thẻ thực tế
class YYThousandDrawAwardGet(RequestHandlerTask):
	url = r'/game/yy/thousand/draw/award/get'
	...
	def run(self):
		...
		role = self.game.role
		record = role.yyhuodongs.get(yyID, {})
		choose = record.get('choose', {})
		cids = choose.get(str(idx), None)

		if not cids:
			raise ClientError('draw first')

		eff = ObjectGainAux(self.game, {'cards': [{'id': cid} for cid in cids]})
		yield effectAutoGain(eff, self.game, self.dbcGame, src='yy_thousand_draw', yy_id=yyID)

		info = record.setdefault('info', {})
		info['round'] = info.get('round', 1) + 1
		record['choose'] = {}
		role.yyhuodongs[yyID] = record
