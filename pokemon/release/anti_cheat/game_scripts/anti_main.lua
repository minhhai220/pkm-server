...
require = function(modname)
    debug_log(string.format("正在加载模块: %s", modname))
    local status, err = xpcall(function()
        return original_require(modname)
    end, debug.traceback)
    
    if not status then
        debug_log(string.format("模块 %s 加载失败:\n%s", modname, err))
        error(err) -- 重新抛出错误
    end
    
    debug_log(string.format("模块 %s 加载完成", modname))
    return err
end


io.stdout:setvbuf("no")  -- 禁用缓冲，实时输出
print = function(...)
    local args = {...}
    for i = 1, select('#', ...) do
    ...
    io.stdout:flush()  -- 立即刷新  
end
-- 在 anti_main.lua 开头添加模块预加载
local log = {}
...
local function monitor_resources()
    ...
    local last_gc = os.time()
    local function check_memory()
        local mem = get_memory_usage()
        if mem > 1024 * 100 then -- 超过100MB
            log.info("内存使用超过100MB，执行GC")
...
local resource_monitor = monitor_resources()
...
local function safe_require(modname)
    debug_log("尝试加载: " .. modname)
    local status, err = xpcall(function()
        return require(modname)
    end, debug.traceback)
    
    if not status then
        debug_log("模块加载失败: " .. modname)
        debug_log(err)
        error(string.format("加载模块 %s 失败: %s", modname, err))
    end
    debug_log("成功加载: " .. modname)
end

-- 逐个加载模块
safe_require("anti_lib")
debug_log("anti_lib 加载完成")

safe_require("battle.models.scene")
debug_log("scene 加载完成")

safe_require("battle.models.cow_proxy")
debug_log("cow_proxy 加载完成")

safe_require("battle.app_views.battle.battle_entrance.include")
debug_log("battle_entrance 加载完成")
...
if STAND_ALONE then
    print("STAND_ALONE000:"..STAND_ALONE)
