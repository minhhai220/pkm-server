...
@coroutine
def createEmerasDB(EmeraIDs, roleID, dbc):
	"""创建琉石数据库数据"""
	if isinstance(EmeraIDs, int):
		EmeraIDs = [EmeraIDs]
	...
	if EmeraIDs:
		...
		for emeraID in EmeraIDs:
			future = dbc.call_async('DBCreate', 'RoleEmera', {
				'role_db_id': roleID,
				'emera_id': emeraID,
			})
			emeraDataFutures.append(future)
		for future in emeraDataFutures:
			emeraData = yield future
			if not emeraData['ret']:
				raise ServerError('db create emera error')
			emeraDatas.append(emeraData['model'])

	raise Return(emeraDatas)

@coroutine
def createContractsDB(ContractIDs, roleID, dbc):
	'''
	创建契约数据库数据
	'''
	contractsData = []
	for csvID in ContractIDs:
		contractData = yield dbc.call_async('DBCreate', 'RoleContract', {
			'role_db_id': roleID,
			'contract_id': csvID,
		})
		if not contractData['ret']:
			raise ServerError('db create contract error')
		contractsData.append(contractData['model'])

	raise Return(contractsData)
...
def effectAutoGain(eff, game, dbc, src=None, mul=None, yy_id=None):
	if eff:
		...
		# 生成新琉石
		if getattr(eff, 'emeraIDs', None):
			emeraDatas = yield createEmerasDB(eff.emeraIDs, game.role.id, dbc)
			eff.setEmerasDBL(emeraDatas)
		# 生成新契约
		if getattr(eff, 'contractIDs', None):
			try:
				contractDatas = yield createContractsDB(eff.contractIDs, game.role.id, dbc)
				eff.setContractsDBL(contractDatas)
			except Exception as e:
				logger.warning("Contract database creation failed: %s", e)
				# 如果契约创建失败，继续处理其他物品
...
def battleCardsAutoDeployment(cardIDs, game, **kwargs):
	...
	# 主城出战队伍、元素挑战、公会战、跨服马戏团、card_embattle中的主战阵容下阵
	game.role.filterBattleCards(cardIDs)

	# 所有助战位置下阵（主城助战、活动助战、竞技场助战等）
	game.role.filterAllAidCards(cardIDs)

	# 养成状态解除（守护徽章、陨石）
	game.role.filterDevelopCards(cardIDs)
	...
	# 陨石守护自动卸下
	meteoriteBattleCardAutomicUnset(game, cardIDs)
...
@coroutine
def createNewWeeklyRecord(weekInt, game, dbc):
	game.role.weekly_record_db_id = None
	weeklyData = yield dbc.call_async('DBCreate', 'WeeklyRecord', {
		'role_db_id': game.role.id,
		'week': weekInt,
	})
	if not weeklyData['ret']:
		raise ServerError('db create weekly record error')

	raise Return(weeklyData)
...
def arenaBattleCardAutomicDeployment(game, cards, **kwargs):
	...
	# 竞技场防守阵容下阵
	validsDefence = game.cards.deploymentForArena.filter('defence_cards', cards)
	if validsDefence:
		from game.handler._pvp import refreshCardsToPVP
		yield refreshCardsToPVP(kwargs['arena'], game, defence_cards=validsDefence)
...
def meteoriteBattleCardAutomicUnset(game, cardIDs):
	"""陨石守护卡牌自动卸下"""
	if not hasattr(game, 'meteorite') or not game.meteorite:
		return
	
	# 遍历所有陨石，检查是否有被删除的卡牌在守护
	for card in game.cards.getCards(cardIDs):
		if hasattr(card, 'meteorite_index') and card.meteorite_index:
			try:
				game.meteorite.unsetCard(card.meteorite_index)
				logger.info('Auto unset card %s from meteorite %d', card.id, card.meteorite_index)
			except Exception as e:
				logger.warning('Failed to auto unset card from meteorite: %s', e)


def meteoriteHeldItemAutomicUnset(game, heldItemIDs):
	"""陨石守护持有物自动卸下"""
	if not hasattr(game, 'meteorite') or not game.meteorite:
		return
	
	# 遍历所有持有物，检查是否有被删除的持有物在守护
	for heldItem in game.heldItems.getHeldItems(heldItemIDs):
		if hasattr(heldItem, 'meteorite_index') and heldItem.meteorite_index:
			try:
				pos = heldItem.meteorite_pos if hasattr(heldItem, 'meteorite_pos') else 0
				if pos:
					game.meteorite.unsetHeldItem(heldItem.meteorite_index, pos)
					logger.info('Auto unset held item %s from meteorite %d pos %d', heldItem.id, heldItem.meteorite_index, pos)
			except Exception as e:
				logger.warning('Failed to auto unset held item from meteorite: %s', e)
