...
def readCsv(fileName):
    with open(fileName, 'rb') as fp:  # 使用 with 自动关闭文件
        content = fp.read()  # 一次性读取更高效
    ...
    if content.startswith(codecs.BOM_UTF8):
        ...
        try:
            ...
            pass  # 如果 UTF-8 解码失败，继续尝试不带 BOM 的情况

    # 尝试用 UTF-8 解码（不带 BOM）
    try:
        return content.decode('utf-8').splitlines()
    except UnicodeDecodeError:
        # 如果 UTF-8 解码失败，回退到 GBK
        try:
            return content.decode('gbk').splitlines()
        except UnicodeDecodeError:
            # 如果 GBK 也失败，使用替代策略（如忽略错误或返回原始字节）
...
def makeLua(csvName, keyList, strMat, defList, hasRepeat):
	...
	module = None
	if len(data) + g_dataMapLen > 50000:
